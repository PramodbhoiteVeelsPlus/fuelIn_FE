...
@Component({
  selector: 'app-advance-tables-widget12',
  templateUrl: './advance-tables-widget12.component.html',
  providers: [
    { provide: NgbDateAdapter, useClass: CustomAdapter },
    { provide: NgbDateParserFormatter, useClass: CustomDateParserFormatter }
  ]
})
export class AdvanceTablesWidget12Component {
  ...
  showCustomer() {
    this.post.getCorporateDetailsForFastagPost()
      .subscribe(res => {
        if (res) {
  ...
  searchPhoneNumber() {
    ...
    this.post.searchPhoneForFuelDealerPost(data)
      .subscribe(res => {
        if (res.status == "OK") {
          if (res.data.length) {
  ...
  submit() {
    if (this.registerCorporateForm.value.state) {
      if (this.registerCorporateForm.value.businessType) {
        if (this.registerCorporateForm.value.thrLimit && this.registerCorporateForm.value.kitNo && this.registerCorporateForm.value.entityId) {
          this.showSpinner()
  ...
  exportAsXLSXCustomer(): void {
    this.excelService.exportAsExcelFile(this.customerDetailData, 'Fastag Report');
  }
