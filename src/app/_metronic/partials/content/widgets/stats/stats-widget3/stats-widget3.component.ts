...
export class StatsWidget3Component {
  page: any = 1;
  pageSize: any = 10;
  page1: any = 1;
  pageSize1: any = 10;
  ...
  showPumpName(month:any, showPump:any) {
    var today = new Date();
    ...
    if (month == 'jan') {
      ...
      startDate = year + '-03-01 00:00:01'
      endDate = year + '-03-31 23:59:59'
    } else if (month == 'apr') {
      startDate = year + '-04-01 00:00:01'
      endDate = year + '-04-30 23:59:59'
    } else if (month == 'may') {
      startDate = year + '-05-01 00:00:01'
      endDate = year + '-05-31 23:59:59'
    } else if (month == 'june') {
      startDate = year + '-06-01 00:00:01'
      endDate = year + '-06-30 23:59:59'
    } else if (month == 'july') {
      startDate = year + '-07-01 00:00:01'
      endDate = year + '-07-31 23:59:59'
    } else if (month == 'aug') {
      startDate = year + '-08-01 00:00:01'
      endDate = year + '-08-31 23:59:59'
    } else if (month == 'sep') {
      startDate = year + '-09-01 00:00:01'
      endDate = year + '-09-30 23:59:59'
    } else if (month == 'oct') {
      startDate = year + '-10-01 00:00:01'
      endDate = year + '-10-31 23:59:59'
    } else if (month == 'nov') {
      startDate = year + '-11-01 00:00:01'
      endDate = year + '-11-30 23:59:59'
    } else if (month == 'dec') {
      startDate = year + '-12-01 00:00:01'
    ...
    this.post.showPumpNameURLPost(data)
      .subscribe(res => {
        if (res.data.length) {
          this.pumpName = res.data
          this.modalRefpass = this.modalService.open(showPump);
  ...
  showCustomerName(month:any, showCust:any) {
    var today = new Date();
    var year = today.getFullYear();
    let startDate
    let endDate
    ...
    this.post.showCustomerNamePost(data)
      .subscribe(res => {
        if (res.data.length) {
          this.custName = res.data
          this.modalRefpass = this.modalService.open(showCust);
          this.modalRefpass.result.then((result:any) => {
            this.closeResult = `Closed with: ${result}`;
          }, (reason:any) => {
            this.closeResult = `Dismissed ${this.getDismissReason(reason)}`;
  ...
  showActiveCustomerName(month:any, showActivePump:any) {
    var today = new Date();
    var year = today.getFullYear();
    let startDate
    let endDate
    if (month == 'jan') {
      ...
      startDate = year + '-02-01 00:00:01'
      endDate = year + '-02-31 23:59:59'
    } else if (month == 'march') {
      startDate = year + '-03-01 00:00:01'
      endDate = year + '-03-31 23:59:59'
    } else if (month == 'apr') {
      startDate = year + '-04-01 00:00:01'
      endDate = year + '-04-31 23:59:59'
    } else if (month == 'may') {
      startDate = year + '-05-01 00:00:01'
      endDate = year + '-05-31 23:59:59'
    } else if (month == 'june') {
    ...
    this.post.showActivePumpNamePost(data)
      .subscribe(res => {
        if (res.data.length) {
          this.activeCustomer = res.data
          this.modalRefpass = this.modalService.open(showActivePump);
          this.modalRefpass.result.then((result:any) => {
            this.closeResult = `Closed with: ${result}`;
          }, (reason:any) => {
            this.closeResult = `Dismissed ${this.getDismissReason(reason)}`;
  ...
  showActiveCustomer(month:any, showActiveCust:any) {
    var today = new Date();
    var year = today.getFullYear();
    let startDate
    let endDate
    ...
    this.post.showActiveCustomerPost(data)
      .subscribe(res => {
        if (res.data.length) {
          this.activeCustomerDatewise = res.data
          this.modalRefpass = this.modalService.open(showActiveCust);
          this.modalRefpass.result.then((result:any) => {
            this.closeResult = `Closed with: ${result}`;
          }, (reason:any) => {
            this.closeResult = `Dismissed ${this.getDismissReason(reason)}`;
  ...
  excelDownloadShowPump() {
    this.pumpNameDetails.length = 0
    this.pumpName.map((res:any) => {
      let json = {
        FuelVeelsId: res.FuelVeelsId,
        VeelsID: res.FuelVeelsId,
        petrolPumpName: res.companyName,
        oilCompany: res.brandName,
        Date: moment(res.corporateCreatedAt).format("DD-MM-YYYY"),
        phoneNumber: res.hostPhone
      };
      this.pumpNameDetails.push(json);
    });

    this.excelService.exportAsExcelFile(
      this.pumpNameDetails,
      "PumpMonthlyDetails"
    );
  }

  excelDownloadShowCustomer() {
    this.customerNameDetails.length = 0
    this.custName.map((res:any) => {
      let json = {
        companyName: res.companyName,
        Date: moment(res.corporateCreatedAt).format("DD-MM-YYYY"),
        phoneNumber: res.hostPhone
      };
      this.customerNameDetails.push(json);
    });

    this.excelService.exportAsExcelFile(
      this.customerNameDetails,
      "CustomerMonthlyDetails"
    );
  }

  excelDownloadActivePump() {
    this.activePumpDetails.length = 0
    this.activeCustomer.map((res:any) => {
      let json = {
        FuelVeelsId: res.FuelVeelsId,
        VeelsID: res.FuelVeelsId,
        petrolPumpName: res.companyName,
        Date: moment(res.chequeDate).format("DD-MM-YYYY"),
        phoneNumber: res.hostPhone
      };
      this.activePumpDetails.push(json);
    });

    this.excelService.exportAsExcelFile(
      this.activePumpDetails,
      "Active Pump"
    );
  }

  excelDownloadActiveCustomer() {
    this.activeCustomerDatewiseDetails.length = 0
    this.activeCustomerDatewise.map((res:any) => {
      let json = {
        companyName: res.companyName,
        Date: moment(res.estimatedRefuelDate).format("DD-MM-YYYY"),
        phoneNumber: res.hostPhone
      };
      this.activeCustomerDatewiseDetails.push(json);
    });
    this.excelService.exportAsExcelFile(
      this.activeCustomerDatewiseDetails,
      "Active customer"
    );
  }
}
