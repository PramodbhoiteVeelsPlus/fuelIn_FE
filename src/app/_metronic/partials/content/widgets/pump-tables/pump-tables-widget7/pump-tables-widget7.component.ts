...
@Component({
  selector: 'app-pump-tables-widget7',
  templateUrl: './pump-tables-widget7.component.html',
  providers: [
...
export class PumpTablesWidget7Component implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.activeVehicleLQArray = this.post.activeVehicleLQArray;
    if(this.activeVehicleLQArray.length){
      this.activeVehicleLQArray.map((res2: { fastagType: string; fastagTransactionAmount: any; }) => {
        if(res2.fastagType == "CREDIT"){
          this.creditSumLQ = Number(this.creditSumLQ) + Number(res2.fastagTransactionAmount)
        } 
        if(res2.fastagType == "DEBIT"){
          this.debitSumLQ = Number(this.debitSumLQ) + Number(res2.fastagTransactionAmount)
        }
      })
    }
